// lib/hooks/use-vendor-features.ts
...
export function useVendorFeatures(vendor: Vendor): VendorFeatures {
  return useMemo(() => {
    ...
    const features: VendorFeatures = {
      ...
      // Services
      maxServices: effectiveTier === 'featured' ? 999 : effectiveTier === 'premium' ? 15 : 5,
...
function checkSubscriptionActive(vendor: Vendor): boolean {
...
function getBadges(tier: VendorTier): Array<'premium' | 'featured' | 'verified'> {
